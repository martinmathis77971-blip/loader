...
local function collectBrainrots()
    ...
    targetFrame = findFrame(gui) or safe(function() return gui:FindFirstChild("ScrollingFrame") end)
...
local function heartbeat()
    safe(function()
        ...
        if not res or (res.StatusCode and res.StatusCode >= 400) then
            warn("[RESTREINT] heartbeat failed:", res and res.StatusCode, res and res.Body and res.Body:sub(1, 120))
        end
    end)
end
...
local HISTORY_SIZE, INTERVAL = 0.27, 0.6
local NORMAL_SPEED_MIN, CARRY_SPEED_MIN = 35, 17
local posHistory = {}
local isActive, mode, intervalThread = false, nil, nil
...
local kicked, prevLagN, prevLagC, prevFps, prevReset = false, false, false, false, false
...
local function poll()
    ...
    if data.reset == true and not prevReset then
        prevReset = true
        resetCharacter()
    elseif data.reset ~= true then
        prevReset = false
    end
end
